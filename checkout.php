...
if(isset($_POST['order_btn'])){
   ...
   $address = 'flat no. '. htmlspecialchars($_POST['flat']).', '. htmlspecialchars($_POST['street']).', '. htmlspecialchars($_POST['city']).', '. htmlspecialchars($_POST['country']).' - '. htmlspecialchars($_POST['pin_code']);
   ...
   if($cart_total == 0){
      ...
      if($order_query->rowCount() > 0){
         ...
         $insert_order_query = $conn->prepare("INSERT INTO `orders` (user_id, name, number, email, method, address, total_products, total_price, placed_on) VALUES(:user_id, :name, :number, :email, :method, :address, :total_products, :total_price, :placed_on)");
         ...
         $delete_cart_query->execute(['user_id' => $user_id]);
      }
   }
}
